...
export function TimePurposeStep({ durationSec, purpose, onChange, onBack, onNext }: Props) {
  ...
  const [customMin, setCustomMin] = useState(isPreset ? 30 : Math.round(durationSec / 60));
  ...
  function applyCustom(min: number) {
    const clamped = Math.max(5, Math.min(180, min));
    setCustomMin(clamped);
    setMode('custom');
    onChange(clamped * 60, purpose);
  }
    ...
    <div>
      ...
      >
        ...
        <input
          ...
          max={180}
          value={customMin}
          onChange={(e) => applyCustom(parseInt(e.target.value, 10) || 5)}
          className="w-20 px-2 py-1 border border-stone-300 rounded"
        />
        <span className="text-sm text-stone-500">분 (5~180)</span>
      </div>
      ...
      <div className="mt-6 flex justify-between">
        ...
        <button
          onClick={onNext}
          className="px-6 py-2 rounded-full bg-[var(--cafe-accent)] text-white font-semibold"
        >
          다음 →
        </button>
      </div>
